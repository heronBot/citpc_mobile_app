...
class LoginScreen(BoxLayout):
    def __init__(self, **kwargs):
        ...
        Clock.schedule_once(self.load_credentials,0)
        # self.load_credentials()
        

    def load_credentials(self,another=None):
        if os.path.exists(self.config_file):
            self.config.read(self.config_file)
            self.ids.username.text = self.config.get('Login', 'username')
            self.ids.password.text = self.config.get('Login', 'password')
...
class LoadingPopup(Popup):
    def __init__(self, **kwargs):
        ...
        box.add_widget(Label(text='Please wait...', size_hint_y=None, height=40))
        self.add_widget(box)

class CITPC_ConnectorApp(App):
    def build(self):
        # Window.size = [300, 600]
